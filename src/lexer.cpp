...
std::ostream &operator<<(std::ostream &os, const TokenType &t) {
  if (t == TokenType::program)
    return os << static_cast<int>(t) << " (PROG)";
  if (t == TokenType::identifier_list)
    return os << static_cast<int>(t) << " (ID_LST)";
  if (t == TokenType::id)
    return os << static_cast<int>(t) << " (ID)";
  if (t == TokenType::declarations)
    return os << static_cast<int>(t) << " (DEC)";
  if (t == TokenType::type)
    return os << static_cast<int>(t) << " (TYPE)";
  if (t == TokenType::standard_type)
    return os << static_cast<int>(t) << " (STD_TYPE)";
  if (t == TokenType::integer)
    return os << static_cast<int>(t) << " (INT)";
  if (t == TokenType::real)
    return os << static_cast<int>(t) << " (REAL)";
  if (t == TokenType::subprogram_declarations)
    return os << static_cast<int>(t) << " (SUB_DECS)";
  if (t == TokenType::subprogram_declaration)
    return os << static_cast<int>(t) << " (SUB_DEC)";
  if (t == TokenType::subprogram_head)
    return os << static_cast<int>(t) << " (SUB_HEAD)";
  if (t == TokenType::function)
    return os << static_cast<int>(t) << " (FUNC)";
  if (t == TokenType::procedure)
    return os << static_cast<int>(t) << " (PROC)";
  if (t == TokenType::arguments)
    return os << static_cast<int>(t) << " (ARGS)";
  if (t == TokenType::parameter_list)
    return os << static_cast<int>(t) << " (PRM_LST)";
  if (t == TokenType::compound_statement)
    return os << static_cast<int>(t) << " (COM_STMT)";
  if (t == TokenType::optional_statements)
    return os << static_cast<int>(t) << " (OPT_STMT)";
  if (t == TokenType::statement_list)
    return os << static_cast<int>(t) << " (STMT_LST)";
  if (t == TokenType::statement)
    return os << static_cast<int>(t) << " (STMT)";
  if (t == TokenType::assignop)
    return os << static_cast<int>(t) << " (ASSIGN)";
  if (t == TokenType::variable)
    return os << static_cast<int>(t) << " (VAR)";
  if (t == TokenType::array)
    return os << static_cast<int>(t) << " (ARRAY)";
  if (t == TokenType::procedure_statement)
    return os << static_cast<int>(t) << " (PROC_STMT)";
  if (t == TokenType::expression_list)
    return os << static_cast<int>(t) << " (EXP_LIST)";
  if (t == TokenType::expression)
    return os << static_cast<int>(t) << " (EXP)";
  if (t == TokenType::relop)
    return os << static_cast<int>(t) << " (RELOP)";
  if (t == TokenType::simple_expression)
    return os << static_cast<int>(t) << " (SIMP_EXP)";
  if (t == TokenType::addop)
    return os << static_cast<int>(t) << " (ADDOP)";
  if (t == TokenType::term)
    return os << static_cast<int>(t) << " (TERM)";
  if (t == TokenType::mulop)
    return os << static_cast<int>(t) << " (MULOP)";
  if (t == TokenType::factor)
    return os << static_cast<int>(t) << " (FACT)";
  if (t == TokenType::sign)
    return os << static_cast<int>(t) << " (SIGN)";
  if (t == TokenType::begin)
    return os << static_cast<int>(t) << " (BEGIN)";
  if (t == TokenType::end)
    return os << static_cast<int>(t) << " (END)";
  if (t == TokenType::t_not)
    return os << static_cast<int>(t) << " (NOT)";
  if (t == TokenType::of)
    return os << static_cast<int>(t) << " (OF)";
  if (t == TokenType::t_if)
    return os << static_cast<int>(t) << " (IF)";
  if (t == TokenType::t_then)
    return os << static_cast<int>(t) << " (THEN)";
  if (t == TokenType::t_else)
    return os << static_cast<int>(t) << " (ELSE)";
  if (t == TokenType::t_while)
    return os << static_cast<int>(t) << " (WHILE)";
  if (t == TokenType::t_do)
    return os << static_cast<int>(t) << " (DO)";
  if (t == TokenType::paren_open)
    return os << static_cast<int>(t) << " (PAREN_O)";
  if (t == TokenType::paren_close)
    return os << static_cast<int>(t) << " (PAREN_C)";
  if (t == TokenType::semicolon)
    return os << static_cast<int>(t) << " (SEMIC)";
  if (t == TokenType::dot)
    return os << static_cast<int>(t) << " (DOT)";
  if (t == TokenType::comma)
    return os << static_cast<int>(t) << " (COMMA)";
  if (t == TokenType::colon)
    return os << static_cast<int>(t) << " (COLON)";
  if (t == TokenType::bracket_open)
    return os << static_cast<int>(t) << " (BRKT_O)";
  if (t == TokenType::bracket_close)
    return os << static_cast<int>(t) << " (BRKT_C)";
  if (t == TokenType::dot_dot)
    return os << static_cast<int>(t) << " (DOT_DOT)";
  if (t == TokenType::eof) 
	return os << static_cast<int>(t) << " (EOF)";
}

struct NoAttrib {
  friend std::ostream &operator<<(std::ostream &os, const NoAttrib &t) {
    return os << "(NULL)";
  }
};

enum class StandardType { integer, real };

std::ostream &operator<<(std::ostream &os, const StandardType &t) {
  if (t == StandardType::integer)
    return os << static_cast<int>(t) << '\t' << "(ST_INT)";
...
std::ostream &operator<<(std::ostream &os, const AddOpType &t) {
  if (t == AddOpType::plus)
    return os << static_cast<int>(t) << '\t' << "(PLUS)";
  if (t == AddOpType::minus)
...
std::ostream &operator<<(std::ostream &os, const MulOpType &t) {
  if (t == MulOpType::mul)
    return os << static_cast<int>(t) << '\t' << "(MUL)";
  if (t == MulOpType::div)
    return os << static_cast<int>(t) << '\t' << "(DIV)";
  if (t == MulOpType::mod)
    return os << static_cast<int>(t) << '\t' << "(MOD)";
...
std::ostream &operator<<(std::ostream &os, const RelOpType &t) {
  if (t == RelOpType::equal)
    return os << static_cast<int>(t) << '\t' << "(EQ)";
  if (t == RelOpType::not_equal)
    return os << static_cast<int>(t) << '\t' << "(NEQ)";
  if (t == RelOpType::less_than)
    return os << static_cast<int>(t) << '\t' << "(LT)";
  if (t == RelOpType::less_than_or_equal)
    return os << static_cast<int>(t) << '\t' << "(LEQ)";
  if (t == RelOpType::greater_than)
    return os << static_cast<int>(t) << '\t' << "(GT)";
  return os << static_cast<int>(t) << '\t' << "(GEQ)";
}

using TokenAttribute = std::variant<NoAttrib, StandardType, AddOpType,
                                    MulOpType, SignOpType, RelOpType>;

std::ostream &operator<<(std::ostream &os, const TokenAttribute &t) {
  if (t.index() == 0)
    return os << std::get<0>(t);
  if (t.index() == 1)
    return os << std::get<1>(t);
  if (t.index() == 2)
    return os << std::get<2>(t);
  if (t.index() == 3)
    return os << std::get<3>(t);
  if (t.index() == 4)
    return os << std::get<4>(t);
}
...
struct TokenInfo {
  ...
  TokenInfo(TokenType type, TokenAttribute attrib, int line, int column)
      : type(type), attrib(attrib), line_location(line),
        column_location(column) {}
...
std::optional<TokenType> ReadTokenTypeFromString(std::string s) {
  ...
  if (s == "of")
    return TokenType::of;
  if (s == "integer")
    return TokenType::standard_type;
  if (s == "real")
    return TokenType::standard_type;
  if (s == "function")
...
std::optional<TokenAttribute> ReadTokenAttributeFromString(std::string s) {
  if (s == "0")
    return NoAttrib{};
  if (s == "div")
    return MulOpType::div;
  if (s == "mod")
    return MulOpType::mod;
  if (s == "and")
    return MulOpType::t_and;
  if (s == "or")
    return AddOpType::t_or;
  if (s == "integer")
    return StandardType::integer;
  if (s == "real")
    return StandardType::real;
  return {};
}
...
std::optional<TokenInfo> CheckReseredWords(ReservedWordList &list,
                                           std::string s) {
  for (auto &item : list) {
    if (item == s)
...
std::ostream &operator<<(std::ostream &os, LexerErrorType t) {
  if (t == LexerErrorType::Id)
    return os << "id";
  if (t == LexerErrorType::Int)
    return os << "int";
  if (t == LexerErrorType::SReal)
    return os << "sReal";
...
std::ostream &operator<<(std::ostream &os, LexerErrorSubType t) {
  if (t == LexerErrorSubType::TooLong)
    return os << "Too Long";
  if (t == LexerErrorSubType::ZeroLength)
    return os << "Zero Length";
  if (t == LexerErrorSubType::TrailingZero)
    return os << "Trailing Zero";
...
struct LexerError {
  ...
  LexerError(LexerErrorType type, LexerErrorSubType subType, int line,
             int column)
      : type(type), subType(subType), line_location(line),
        column_location(column) {}
...
using LexerMachineFuncSig =
    std::function<std::optional<LexerMachineReturn>(ProgramLine &line)>;
...
public:
  ...
  void TokenFilePrinter(int line_num, std::string lexeme,
                        LexerMachineReturn::ReturnVariant content);
...
void Lexer::AddMachine(LexerMachine &&machine) {
  ...
  std::sort(std::begin(machines), std::end(machines),
            [](LexerMachine a, LexerMachine b) {
              return a.precedence > b.precedence;
            });
}

void Lexer::TokenFilePrinter(int line_num, std::string lexeme,
                             LexerMachineReturn::ReturnVariant content) {
  if (content.index() == 1) {
...
std::vector<TokenInfo> Lexer::GetTokens(std::vector<std::string> lines) {
  ...
  for (auto &s_line : lines) {
    ...
    while (backward_index < full_line.size()) {
      buffer.clear();
      buffer.insert(std::begin(buffer), std::begin(full_line) + backward_index,
                    std::end(full_line));
      ...
      if (machine_ret.has_value() && iter != std::end(machines)) {
        if (machine_ret->chars_to_eat > 0 &&
          ...
          if (machine_ret->content.index() == 1) {
            ...
            std::get<2>((machine_ret->content)).column_location =
                backward_index;
...
void Lexer::CreateMachines() {
  AddMachine({"Comment", 110,
              [&](ProgramLine &line) -> std::optional<LexerMachineReturn> {
       ...
       [&](ProgramLine &line) -> std::optional<LexerMachineReturn> {
         if (line.size() >= 1 && std::isalpha(line[0])) {
           ...
           if (index <= identifier_length) {
             auto res_word = CheckReseredWords(
                 reservedWords, Str_ProgramLine(Sub_ProgramLine(line, index)));
             if (res_word.has_value()) {
               return LexerMachineReturn(index, res_word.value());
             }
             return LexerMachineReturn(index,
                                       TokenInfo(TokenType::id, NoAttrib()));
           }
           return LexerMachineReturn(
               index, LexerError(LexerErrorType::Id, LexerErrorSubType::TooLong,
  ...
  AddMachine({"Catch-all", 80,
              [](ProgramLine &line) -> std::optional<LexerMachineReturn> {
                if (line.size() >= 2) {
                  if (line[0] == ':' && line[1] == '=')
                    return LexerMachineReturn(
                        2, TokenInfo(TokenType::assignop, NoAttrib()));
                  if (line[0] == '.' && line[1] == '.')
                    return LexerMachineReturn(
                        2, TokenInfo(TokenType::dot_dot, NoAttrib()));
                }
                if (line.size() >= 1) {
			      if (line[0] == '\0')
				    return LexerMachineReturn (
						1, TokenInfo (TokenType::eof, NoAttrib ()));
                  if (line[0] == '(')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::paren_open, NoAttrib()));
                  if (line[0] == ')')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::paren_close, NoAttrib()));
                  if (line[0] == ';')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::semicolon, NoAttrib()));
                  if (line[0] == '.')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::dot, NoAttrib()));
                  if (line[0] == ',')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::comma, NoAttrib()));
                  if (line[0] == ':')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::colon, NoAttrib()));
                  if (line[0] == '[')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::bracket_open, NoAttrib()));
                  if (line[0] == ']')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::bracket_close, NoAttrib()));
                  if (line[0] == '+')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::sign, SignOpType::plus));
                  if (line[0] == '-')
                    return LexerMachineReturn(
                        1, TokenInfo(TokenType::sign, SignOpType::minus));
                }
                return {};
              }});
  AddMachine(
      {"Real", 40, [](ProgramLine &line) -> std::optional<LexerMachineReturn> {
         return {};
       }});
  AddMachine({"Integer", 60,
              [](ProgramLine &line) -> std::optional<LexerMachineReturn> {
                int i = 0;
                ...
                if (i > 0)
                  return LexerMachineReturn(
                      i, TokenInfo(TokenType::integer, NoAttrib()));
                return {};
              }});
  AddMachine(
      {"Relop", 50, [](ProgramLine &line) -> std::optional<LexerMachineReturn> {
         if (line.size() >= 2) {
           if (line[0] == '>' && line[1] == '=')
             return LexerMachineReturn(
                 2,
                 TokenInfo(TokenType::relop, RelOpType::greater_than_or_equal));
           if (line[0] == '<' && line[1] == '=')
             return LexerMachineReturn(
                 2, TokenInfo(TokenType::relop, RelOpType::less_than_or_equal));
           if (line[0] == '<' && line[1] == '>')
             return LexerMachineReturn(
                 2, TokenInfo(TokenType::relop, RelOpType::not_equal));
         }
         if (line[0] == '>')
           return LexerMachineReturn(
               1, TokenInfo(TokenType::relop, RelOpType::greater_than));
         if (line[0] == '<')
           return LexerMachineReturn(
               1, TokenInfo(TokenType::relop, RelOpType::less_than));
         if (line[0] == '=')
           return LexerMachineReturn(
               1, TokenInfo(TokenType::relop, RelOpType::equal));
         return {};
       }});
}
