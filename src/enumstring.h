...
template <typename T> std::istream &operator>> (std::istream &str, enumRefHolder<T> const &data)
{
	std::string value;
	str >> value;
...
template <typename T> enumRefHolder<T> enumFromString (T &e) { return enumRefHolder<T> (e); }